...
#define B(PC, DEST) \
    ((b_enc){.bits = {.cond = 0b1110, .enc = 0b101, .l = 0, .imm24 = (((intptr_t)DEST - (intptr_t)PC) / 4) - 2}})
#define LDR_OFFS(RT, RN, IMM)                   \
    ((ldst_enc){.bits = {.cond    = 0b1110,     \
                         .enc     = 0b010,      \
                         .p       = 1,          \
                         .u       = (IMM >= 0), \
                         .b       = 0,          \
...
int _so_load(so_module* mod, SceUID so_blockid, void* so_data, uintptr_t load_addr)
{
    ...
    for (int i = 0; i < mod->ehdr->e_phnum; i++)
    {
        if (mod->phdr[i].p_type == PT_LOAD)
        {
            ...
            else
            {
                ...
                prog_size = ALIGN_MEM(mod->phdr[i].p_memsz + mod->phdr[i].p_vaddr - (data_addr - mod->text_base),
                                      mod->phdr[i].p_align);
                ...
                res         = mod->data_blockid[mod->n_data] =
            ...
            kuKernelCpuUnrestrictedMemcpy((void*)mod->phdr[i].p_vaddr,
                                          (void*)((uintptr_t)so_data + mod->phdr[i].p_offset), mod->phdr[i].p_filesz);
        }
    }
    ...
    if (mod->dynamic == NULL || mod->dynstr == NULL || mod->dynsym == NULL || mod->reldyn == NULL ||
        mod->relplt == NULL)
    {
        res = -2;
        goto err_free_data;
...
int so_resolve_with_dummy(so_module* mod, so_default_dynlib* default_dynlib, int size_default_dynlib,
